...
interface ThemeInputFormProps {
  onGenerate: (theme: string, numSlides: number) => Promise<void>;
  loading: boolean;
}

export function ThemeInputForm({ onGenerate, loading }: ThemeInputFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await onGenerate(theme, numSlides);
  };
    ...
    >
      <div className="rounded-lg border border-orange-200 bg-orange-50 p-6">
        ...
        <p className="mb-6 text-sm text-gray-500">
          テーマを入力するだけで、AIがスライドの内容を自動生成します
        </p>
